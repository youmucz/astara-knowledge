...
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler, g *rbacGuards) {
	...
	kbRead := kb.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		// The embedded (Plane-hosted) surface never reaches native source
		// documents, so this stays behind the same native-source guard as
		// the listing routes it selects from.
		kbRead.POST("/batch-download", g.Contributor(), middleware.RequireNativeSourceRoute(), g.KBAccessWrite("id"), handler.BatchDownloadKnowledge)
