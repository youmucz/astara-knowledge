...
func TestKnowledgePostProcessGraphEnqueueSelection(t *testing.T) {
	...
	// Graph extraction is a prohibited feature under the astara knowledge-only
	// profile, which suppresses graph enqueueing; this test pins the
	// unrestricted-profile behaviour, so opt out of the profile explicitly.
	t.Setenv(astara.ProfileEnv, "default")
