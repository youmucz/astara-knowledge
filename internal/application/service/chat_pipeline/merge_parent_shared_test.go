...
func TestResolveParentChunksResolvesSharedKBParentsAcrossTenants(t *testing.T) {
	...
	// All chunks belong to workspace 2 and to the same document as the hit;
	// the caller is workspace 1, so only tenant scoping is under test here.
	repo := &tenantScopedChunkRepo{expandChunkRepo: &expandChunkRepo{
		chunks: map[string]*types.Chunk{
			"parent": {
				ID: "parent", TenantID: 2, KnowledgeID: "doc", ChunkType: types.ChunkTypeParentText,
				Content: "shared parent body with ![img](resource://sharedImage)",
			},
			"child": {
				ID: "child", TenantID: 2, KnowledgeID: "doc", ChunkType: types.ChunkTypeText,
		...
		children: map[string][]*types.Chunk{
			"child": {
				{
					ID: "imageChild", TenantID: 2, KnowledgeID: "doc", ParentChunkID: "child",
